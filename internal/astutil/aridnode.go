...
var loggerFuncIdentifiers = map[string]bool{
	"Infof":  true,
	"Errorf": true,
	"Warnf":  true,
	"Debugf": true,
...
func IsAridNode(node ast.Node) bool {
	...
	switch n := node.(type) {
	case *ast.ExprStmt:
		if isLoggerStmt(node) || isLoggerFuncStmt(node) {
			return true
		}
...
func isLoggerFuncStmt(es ast.Node) bool {
	firstIdent := ""
	ast.Inspect(es, func(n ast.Node) bool {
		if fun, ok := n.(*ast.SelectorExpr); ok {
			// Since it's a depth first traversal, first identifier should be
			// one of loggerFuncIdentifiers for the statement to be a logger stmt.
			if firstIdent != "" {
				// If we've already found our candidate don't recurse further
				return false
			}

			firstIdent = fun.Sel.Name
			return false
		}
	...
	_, found := loggerFuncIdentifiers[firstIdent]
